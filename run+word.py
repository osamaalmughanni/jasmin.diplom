...
def generate_pdf():
    ...
    Zusätzlich wird ein extrem sauberes DOCX-Dokument (da.docx) erzeugt, das alle Markdown-Dateien
    (ohne LaTeX-spezifische Anpassungen) beinhaltet. Die DOCX-Datei wird neben der PDF abgelegt.
    """
...
"""
    ...
    ########################################################################
    # 4) DOCX-Dokument erzeugen
    ########################################################################
    try:
        # Für das DOCX wollen wir die ursprünglichen Markdown-Dateien verwenden,
        # ohne die LaTeX-spezifischen Ersetzungen. Zwischen den Dateien wird ein
        # HTML-Block als Seitenumbruch eingefügt.
        combined_md_docx = ""
        for i, md_file in enumerate(md_files):
            with open(md_file, "r", encoding="utf-8") as f:
                content = f.read()
            combined_md_docx += content
            if i < len(md_files) - 1:
                # Dieser HTML-Block wird von Pandoc als Seitenumbruch interpretiert.
        ...
        subprocess.run(cmd_docx, input=combined_md_docx, text=True, check=True, encoding="utf-8")
        print(f"✅ DOCX Dokument erstellt: {output_docx}")
    except subprocess.CalledProcessError as e:
        print(f"❌ Fehler bei der Erstellung des DOCX Dokuments: {e}")
